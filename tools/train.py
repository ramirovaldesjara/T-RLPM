...
from pm.utils import plot_metrics,plot_cumulative_returns_simple
...
def main():
    ...
    for episode in range(start_episode + 1, cfg.num_episodes + 1):
        ...
        # Convert infos to JSON-safe format
        def recursive_convert(obj):
            if isinstance(obj, np.ndarray):
                return obj.tolist()
            elif isinstance(obj, (np.generic, np.float32, np.float64)):
                return float(obj)
        ...
        with pathmgr.open(os.path.join(exp_path, "train_log.txt"), "a") as op:
            ...
            op.write(json.dumps(test_episode_log_stats) + "\n")

        with pathmgr.open(os.path.join(exp_path, "train_infos.txt"), "a") as op:
            op.write(json.dumps(recursive_convert(infos)) + "\n")
    #
    ...
    test_stats, infos = validate(test_envs, agent)
    ...
    info = infos['portfolio_values']
    dates = infos['date'][0]
    plot_path = os.path.join(exp_path, "cumulative_returns.png")
    plot_cumulative_returns_simple(info, dates, plot_path)
