...
#        self.alpha_log = state_dict["alpha_log"]
    ...
    def validate_net(self, environment):
        ...
        while True:
            ...
            #     infos["portfolio_rets"][i].append(info['portfolio_ret'])
            #     infos["portfolio_values"][i].append(info['portfolio_value'])
            #     infos["date"][i].append(info['date'])

            for i in range(num_envs):
                env_info = None

                # Try to access info directly
                if "portfolio_ret" in info and isinstance(info["portfolio_ret"], np.ndarray):
                    try:
                        env_info = {
                            "portfolio_ret": info["portfolio_ret"][i],
                            "portfolio_value": info["portfolio_value"][i],
                            "date": info["date"][i],
                        }
                    except IndexError:
                        env_info = None

                # If info not available, fall back to final_info
                if env_info is None and "final_info" in info and info["final_info"][i] is not None:
                    env_info = info["final_info"][i]
                    # print(f"[INFO] Using final_info for env {i}")

                if env_info:
                    infos["portfolio_rets"][i].append(env_info.get("portfolio_ret", 0.0))
                    infos["portfolio_values"][i].append(env_info.get("portfolio_value", 0.0))
                    infos["date"][i].append(env_info.get("date", "NA"))
                else:
                    print(f"[WARN] Missing info for env {i} — appending defaults.")
                    infos["portfolio_rets"][i].append(0.0)
                    infos["portfolio_values"][i].append(0.0)
                    infos["date"][i].append("NA")
